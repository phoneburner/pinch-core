...
final class BinaryMemoryUnitTest extends TestCase
{
    ...
    public static function provideFitValues(): \Generator
    {
        ...
        yield 'Large Value Beyond EiB' => [2 * (1024 ** 6), BinaryMemoryUnit::Exbibyte];
        yield 'Very Large Negative Value' => [-2 * (1024 ** 6), BinaryMemoryUnit::Exbibyte];
    }

    #[Test]
    public function baseConstantHasCorrectValue(): void
    {
        self::assertSame(1024, BinaryMemoryUnit::BASE);
        self::assertSame(BinaryMemoryUnit::BASE, 2 ** 10);
    }

    #[Test]
    public function fitHandlesEdgeCases(): void
    {
        // Test with PHP_INT_MAX - should still return Exbibyte
        self::assertSame(BinaryMemoryUnit::Exbibyte, BinaryMemoryUnit::fit(\PHP_INT_MAX));

        // Test with PHP_INT_MIN - should return Exbibyte (uses abs())
        self::assertSame(BinaryMemoryUnit::Exbibyte, BinaryMemoryUnit::fit(\PHP_INT_MIN));
    }
}
