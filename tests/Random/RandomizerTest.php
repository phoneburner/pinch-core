...
use PhoneBurner\Pinch\Tests\Fixtures\EmptyEnum;
...
final class RandomizerTest extends TestCase
{
    ...
    #[Test]
    public function keysReturnsSingleKeyWhenNumIsOne(): void
    {
        $array = ['a' => 1, 'b' => 2, 'c' => 3, 'd' => 4, 'e' => 5];
        $keys = $this->randomizer->keys($array, 1);
        self::assertCount(1, $keys);
        self::assertArrayHasKey($keys[0], $array);
    }
    ...
    #[Test]
    public function shuffleReturnsShuffledString(): void
    {
        $string = 'abcdef';
        $shuffled = $this->randomizer->shuffle($string);
        self::assertIsString($shuffled);
        self::assertSame(\strlen($string), \strlen($shuffled));

        // Check that all original characters are present
        $original_chars = \str_split($string);
        $shuffled_chars = \str_split($shuffled);
        \sort($original_chars);
        \sort($shuffled_chars);
        self::assertSame($original_chars, $shuffled_chars);
    }

    #[Test]
    public function shuffleHandlesEmptyString(): void
    {
        self::assertSame('', $this->randomizer->shuffle(''));
    }

    #[Test]
    public function shuffleHandlesEmptyArray(): void
    {
        self::assertSame([], $this->randomizer->shuffle([]));
    }
